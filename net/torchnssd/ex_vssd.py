...
def mini_chunk_scan_combined(X, dt, A, B, C, chunk_size: int, initial_states):
    ...
    Y, final_state = ssd_minimal_discrete_no_init(X * dt.unsqueeze(-1), A * dt, B, C, chunk_size)
    return Y
...
class ConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, dilation=1, groups=1,
        ...
        self.dropout = nn.Dropout2d(dropout, inplace=False) if dropout > 0 else nn.Identity()
        self.conv = nn.Conv2d(
            in_channels,
            out_channels,
...
class Mamba2(nn.Module):
    def __init__(
        ...
        self.kwargs = kwargs
        self.bidirectional = True if kwargs.get('bidirectional') != None else False
    ...
    def forward(self, u, H: int, W: int):
        ...
        initial_states = self.init_states[None].repeat(batch, 1, 1, 1) if self.learnable_init_states else torch.zeros(0, device=u.device)
        ...
        if self.linear_attn_duality:
            y = self.non_casual_linear_attn(
                ...
                x.reshape(x.shape[0], x.shape[1], x.shape[2] // self.headdim, self.headdim),
                dt, A, B, C, self.D, H, W
            )
        else:
            if self.bidirectional:
                ...
                dts = dt.chunk(2, dim=-1)  # (B, L, nheads) -> (B, L, nheads//2)*2
                As, Ds = A.chunk(2, dim=-1), self.D.chunk(2, dim=-1)  # (nheads) -> (nheads//2)*2
