...
class upsampling(nn.Module):   #反卷积 size翻倍
    def __init__(self, in_channels, out_channels):
...
class ReconNet(nn.Module):
    def __init__(self, channels):
        ...
        self.conv4 = nn.Conv2d(channels, 1, kernel_size=(1, 1), stride=(1, 1), padding=0)
        self.retinex = Retinex_decom(channels)
        self.relu = nn.LeakyReLU()

    def forward(self, l, r):

        l_down2, l_down4, l_down8 = self.pyramid(l)  #前3通道
        l_down80 = self.channel_down(l_down8)  # l

        r_down2, r_down4, r_down8 = self.pyramid(r)  #后3
        r_down80 = self.channel_down(r_down8)  # R

        l0, r0 = self.retinex(l_down80, r_down80)
        # l0: torch.Size([1, 3, 64, 64])
        # r0: torch.Size([1, 3, 64, 64])

        l0 = self.channel_up(l0)
        r0 = self.channel_up(r0)

        l_up2 = self.up_sampling0(
            self.block_up1(self.block_up0(l0) + l_down8))
        l_up4 = self.up_sampling1(
            self.block_up3(self.block_up2(l_up2) + l_down4))
        l1 = self.up_sampling2(
            self.block_up5(self.block_up4(l_up4) + l_down2))

        r_up2 = self.up_sampling0(
            self.block_up1(self.block_up0(r0) + r_down8))
        r_up4 = self.up_sampling1(
            self.block_up3(self.block_up2(r_up2) + r_down4))
        r1 = self.up_sampling2(
            self.block_up5(self.block_up4(r_up4) + r_down2))



        r = self.conv3(self.relu(self.conv2(r1)))
        l = self.conv4(self.relu(self.conv2(l1)))
...
class CTDN(nn.Module):
    def __init__(self, channels=64):
        ...
        # 初始化子模块 ReconNet 和 Retinex_decom
        self.ReconNet = ReconNet(channels)
        self.retinex = Retinex_decom(channels)

    def forward(self, l, r):
        # 1 6 512 512
        # images512: torch.Size([1, 6, 512, 512])

        low_L, low_R, img = self.ReconNet(l,r)

        return low_L, low_R, img
